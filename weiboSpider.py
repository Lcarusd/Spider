...
class Weibo(object):
    cookie = {'Cookie': 'ALF=1568527304; SCF=Alxev4NehK1gtpqF8k_43LPtzVy2XGrbWT2XxUP30JfF1r-Bjt-dcHrSQkQvPjqpAW__WIdqEkSZThHF3ljRgYg.; SUB=_2A25wUjaZDeRhGeVM6FER-SrEwjmIHXVTvVrRrDV6PUNbktANLUv1kW1NTiYgMC8OgJMOvH_KAGlyRAZK0_9cyNKV; SUBP=0033WrSXqPxfM725Ws9jqgMF55529P9D9W5qoTZhPgNBw8FW8g6KcNsZ5JpX5KzhUgL.FoeEe0e71KBR1K-2dJLoI7Ueqc8EUPS.; SUHB=0FQ7NrnpNh-CqG; SSOLoginState=1565935305; MLOGIN=1; _T_WM=10954559625; M_WEIBOCN_PARAMS=lfid%3D102803%26luicode%3D20000174'}
...
def main():
    try:
        ...
        user_id = 2134671703  # 可以改成任意合法的用户id（爬虫的微博id除外）
        filter = 1  # 值为0表示爬取全部微博（原创微博+转发微博），值为1表示只爬取原创微博
        pic_download = 1  # 值为0代表不下载微博原始图片,1代表下载微博原始图片
        video_download = 1  # 值为0代表不下载微博视频,1代表下载微博视频
        wb = Weibo(user_id, filter, pic_download, video_download)  # 调用Weibo类，创建微博实例wb
        wb.start()  # 爬取微博信息
